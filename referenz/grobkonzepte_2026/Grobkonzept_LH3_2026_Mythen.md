# Grobkonzept: Neue Lehrkräftehandreichung

## Arbeitstitel
**„Gesundheitsmythen im Netz – Was stimmt wirklich?"**

*Alternativtitel: „Fakt oder Fake? Gesundheitsbehauptungen kritisch prüfen"*

---

## 1. Rahmendaten

| Aspekt | Beschreibung |
|--------|--------------|
| **Zielgruppe** | Lehrkräfte weiterführender Schulen (Sekundarstufe I + II) |
| **Altersgruppe Lernende** | 11–18 Jahre (mit Binnendifferenzierung) |
| **Dauer** | 90 Minuten (Doppelstunde) |
| **Anlass** | Zunahme gesundheitsbezogener Falschinformationen in Social Media; unrealistische Körperbilder als Gesundheitsrisiko |
| **DSGVO-Hinweis** | Alle Methoden ohne Registrierung/personenbezogene Daten umsetzbar; Arbeit mit fiktiven Beispielen |

---

## 2. Leitfrage

> **Wie erkenne ich verlässliche Gesundheitsinformationen im Netz – und warum ist das für meine körperliche und mentale Gesundheit wichtig?**

---

## 3. Kernbotschaften

1. **Nicht alles, was viral geht, ist wahr** – Gesundheitsmythen verbreiten sich online besonders schnell, weil sie einfache Lösungen versprechen.
2. **Körperbilder und Gesundheitsideale sind oft konstruiert** – Social Media zeigt unrealistische, teilweise gesundheitsschädliche Standards („Traumkörper in 4 Wochen").
3. **Wissenschaftliche Quellen erkennen schützt** – Wer Fakten von Mythen unterscheiden kann, trifft bessere Gesundheitsentscheidungen.
4. **Kritisches Denken ist Gesundheitsschutz** – Medienkompetenz befähigt zu informierten Entscheidungen über den eigenen Körper und schützt vor gefährlichen Trends.

---

## 4. Abgrenzung zu bestehenden Handreichungen

| Bestehende LH | Neue Handreichung |
|---------------|-------------------|
| **LH6** Desinformation: Fake News allgemein | Gesundheitsspezifische Falschinformationen |
| **LH19** Körperbilder: Schönheitsideale & Social Media | Gesundheitsmythen zu Körper, Ernährung, „Wellness" |
| **LH8** Digital stark: Gesunde Gewohnheiten entwickeln | Kritische Bewertung von Gesundheitsbehauptungen |
| **LH7** Influencer:innen: Werbung erkennen | Gesundheits-Influencer:innen & ihre Aussagen prüfen |
| Fokus: Allgemeine Medienbildung | Fokus: Gesundheitskompetenz & kritische Quellenbewertung |

**Verbindung:** Die neue Handreichung dockt an LH6 (Methoden zur Faktenprüfung), LH19 (Körperbilder) und LH8 (Gesundheitsthemen) an und erweitert um die kritische Bewertung gesundheitsbezogener Online-Inhalte – mit besonderem Fokus auf die Verbindung von Körperbildern und Gesundheitsmythen.

---

## 5. Anknüpfungspunkte an bestehende Einheiten

| Einheit | Anknüpfung |
|---------|------------|
| **LH6** Desinformation im Netz | Methoden zur Faktenprüfung (werden vorausgesetzt/kurz aktiviert) |
| **LH19** Körperbilder im Internet | Unrealistische Körperideale, Bildmanipulation |
| **LH8** Digital stark: Gesundheit entdecken | Ernährung, Bewegung, Schlaf als Grundthemen |
| **LH7** Die Welt der Influencer:innen | Werbung vs. Information, kommerzielle Interessen |
| **LH21** Medien und Prävention | Gesundheitsinformationen finden und bewerten |
| **LH14** Challenges im Netz | Gefährliche Trends, Gruppendruck |

---

## 6. Gesundheitsbezug (explizit)

### Körperliche Gesundheit
- Gefährliche „Gesundheitstrends" (extreme Diäten, fragwürdige Fitness-Challenges, „Detox"-Produkte)
- Risiken durch Falschinformationen zu Ernährung, Nahrungsergänzung, Medikamenten
- Unrealistische Körperbilder als Motivation für ungesunde Verhaltensweisen (Essstörungen, Übertraining)

### Mentale Gesundheit
- Druck durch idealisierte Körperbilder und Gesundheitsstandards
- Verunsicherung durch widersprüchliche Gesundheitsinformationen
- Selbstwertprobleme durch Vergleich mit „perfekten" Online-Körpern
- Schuldgefühle, wenn man „Gesundheitstrends" nicht folgt

### Medienkompetenz als Schutzfaktor
- Wissenschaftliche Quellen von Meinungen unterscheiden
- Kommerzielle Interessen hinter Gesundheitsbehauptungen erkennen
- Selbstbestimmte, informierte Entscheidungen über die eigene Gesundheit treffen
- „Gesundheits-Bullshit" als solchen erkennen

---

## 7. KMK-Kompetenzbereiche

| Bereich | Bezug |
|---------|-------|
| **1. Suchen, Verarbeiten, Aufbewahren** | 1.2 Informationen und Daten analysieren, interpretieren und kritisch bewerten |
| **2. Kommunizieren und Kooperieren** | 2.4 An der Gesellschaft aktiv teilhaben |
| **3. Produzieren und Präsentieren** | 3.1 Mehrere technische Bearbeitungswerkzeuge kennen und anwenden (Quiz erstellen) |
| **4. Schützen und sicher Agieren** | 4.1 Sicher in digitalen Umgebungen agieren; 4.4 Gesundheit schützen |
| **5. Problemlösen und Handeln** | 5.5 Algorithmen erkennen und formulieren |
| **6. Analysieren und Reflektieren** | 6.1 Medien analysieren und bewerten; 6.2 Medien in der digitalen Welt verstehen und reflektieren |

**Schwerpunkt:** Bereiche 1, 4 und 6

---

## 8. Dimensionen digitaler Gesundheitskompetenz

| Dimension | Bezug |
|-----------|-------|
| **1. Zugang** | ○ |
| **2. Verstehen** | ● Verstehen, wie Gesundheitsmythen entstehen und sich verbreiten |
| **3. Bewerten** | ● Kritisches Bewerten von Gesundheitsinformationen und Quellen |
| **4. Anwenden** | ● Faktencheck-Methoden anwenden, verlässliche Quellen nutzen |
| **5. Schützen** | ● Gesundheit durch informierte Entscheidungen schützen |
| **6. Kommunizieren** | ○ Gesundheitsinformationen verantwortungsvoll teilen |
| **7. Produzieren** | ● Eigene wissenschaftsbasierte Quiz-Inhalte erstellen |

**Schwerpunkt:** Dimensionen 2, 3, 4, 5, 7

---

## 9. Ablaufstruktur (90 Minuten)

### Phase 1: Einstieg – „Glaubst du das?" (15 Min.)
**Ziel:** Vorwissen aktivieren, persönliche Relevanz herstellen, Neugier wecken

| Element | Beschreibung | Sozialform | Material |
|---------|--------------|------------|----------|
| Impuls | Bildcollage: 8 Gesundheitsbehauptungen aus Social Media (Mix aus Fakt/Fake) | Plenum | Bildmaterial/Folie |
| Positionierung | „Daumen hoch/runter" – spontane Einschätzung zu jeder Behauptung | Plenum | — |
| Sammlung | Eigene Erfahrungen: „Welche Gesundheitstipps habt ihr online gesehen?" | Think-Pair-Share | Karten/Padlet |

**Beispiel-Behauptungen für Einstieg:**
- „Detox-Tees entgiften deinen Körper"
- „10.000 Schritte am Tag sind ein Muss"
- „Blaues Licht vom Handy macht blind"
- „Mit diesem Smoothie nimmst du 5 kg in einer Woche ab"

**Gesundheitsbezug:** Im Anschluss an die Positionierung kurze Brückenfrage einbauen: „Welche dieser Behauptungen könnte eure Gesundheitsentscheidungen beeinflussen – z. B. was ihr esst, trinkt oder kauft?" Damit wird von Beginn an klar, dass es nicht um abstraktes Faktenwissen geht, sondern um den eigenen Körper und die eigene Gesundheit.

**Differenzierung Sek I/II:**
- Sek I: Fokus auf TikTok, Instagram, YouTube (Beauty, Fitness, „Life Hacks")
- Sek II: Zusätzlich: Gesundheitsblogs, Influencer-Marketing, pseudowissenschaftliche Studien

---

### Phase 2: Hinführung – Warum verbreiten sich Mythen? (15 Min.)
**Ziel:** Mechanismen der Verbreitung verstehen, Verbindung zu Körperbildern herstellen

| Element | Beschreibung | Sozialform | Material |
|---------|--------------|------------|----------|
| Input | Kurzvideo/Infografik: „Warum verbreiten sich Gesundheitsmythen so schnell?" | Plenum | Video/Infografik |
| Erarbeitung | Gründe sammeln: Algorithmen, Emotionen, vereinfachte Botschaften, Körperideale | Partnerarbeit | Arbeitsblatt 1 |
| Sicherung | Ergebnisse im Plenum, Kernfrage: „Welche Rolle spielen Körperbilder?" | Plenum | Tafel/Board |

**Schlüsselerkenntnisse:**
- Einfache Lösungen für komplexe Probleme versprechen („In 4 Wochen zum Sixpack")
- Emotionen ansprechen (Angst, Hoffnung, Scham)
- Algorithmen bevorzugen Engagement → kontroverse/emotionale Inhalte
- Idealisierte Körperbilder als „Beweis" für Wirksamkeit
- **Psychische Gesundheitsfolge:** Widersprüchliche oder beängstigende Gesundheitsinfos erzeugen Verunsicherung, Stress und Schamgefühle – die Verbreitung von Mythen ist kein harmloses Phänomen, sondern hat direkte Auswirkungen auf das Wohlbefinden

**Differenzierung Sek I/II:**
- Sek I: Konkrete Beispiele (Before-After-Bilder, „Wundermittel", Challenges)
- Sek II: Zusätzlich: Psychologische Mechanismen (Bestätigungsfehler, Dunning-Kruger-Effekt, Sozialer Beweis)

---

### Phase 3: Erarbeitung I – Faktencheck-Werkstatt (25 Min.)
**Ziel:** Praktische Methoden zur Überprüfung von Gesundheitsbehauptungen erlernen und anwenden

| Element | Beschreibung | Sozialform | Material |
|---------|--------------|------------|----------|
| Methodenkoffer | QQQQ-Methode kennenlernen + Übersicht verlässlicher Quellen | Plenum | Arbeitsblatt 2 |
| Praxis | 4 Gesundheitsbehauptungen in Gruppen überprüfen | Gruppenarbeit (3-4) | Arbeitsblatt 3a-d |
| Präsentation | Jede Gruppe präsentiert einen Faktencheck (je 3 Min.) | Plenum | — |

**QQQQ-Methode (kindgerecht: „Die 4-Fragen-Methode"):**
- **Q**uelle: Wer sagt das? Hat die Person Fachwissen?
- **Q**ualität: Gibt es echte Studien oder nur Meinungen?
- **Q**uerbezüge: Was sagen andere (unabhängige) Quellen?
- **Q**ualitätssiegel: Gibt es unabhängige Prüfung (z.B. Gesundheitsportale)?

**Beispiel-Behauptungen für Faktenchecks:**
1. „Detox-Tees entgiften deinen Körper" → **Mythos** (Leber/Nieren übernehmen das)
2. „7-8 Stunden Schlaf sind gesund" → **Fakt** (mit individuellen Unterschieden)
3. „Mit dieser 5-Minuten-Übung zum Sixpack" → **Mythos** (unrealistisch, oft nur Marketing)
4. „Zucker macht süchtig wie Drogen" → **Differenziert** (Vereinfachung, aber Suchtpotential vorhanden)

**Differenzierung Sek I/II:**
- Sek I: Vereinfachte QQQQ-Checkliste, klare Mythen vs. Fakten, vorgegebene Quellen
- Sek II: Zusätzlich: Studienqualität bewerten, Interessenkonflikte erkennen, eigene Recherche

---

### Phase 4: Erarbeitung II – Körperbilder & Gesundheitsmythen (15 Min.)
**Ziel:** Verbindung zwischen konstruierten Körperbildern und Gesundheitsmythen erkennen

| Element | Beschreibung | Sozialform | Material |
|---------|--------------|------------|----------|
| Fallanalyse | 3 fiktive Influencer-Profile: Fitness / Ernährung / Beauty | Gruppenarbeit | Arbeitsblatt 4 |
| Leitfragen | „Welches Körperbild wird vermittelt? Welche Mythen? Wer profitiert?" | Gruppenarbeit | — |
| Reflexion | Plenum: „Wie realistisch sind diese Ideale? Welche Risiken entstehen?" | Plenum | — |

**Fiktive Profile (Beispiele):**
1. **@FitLife_Max** – Fitness-Influencer mit „Vorher-Nachher"-Transformation, verkauft Trainingsplan
2. **@CleanEating_Lisa** – Ernährungs-Influencerin mit strengen Regeln, bewirbt Supplements
3. **@Glow_Queen** – Beauty-Influencerin mit „natürlichem" Glow, verkauft Detox-Produkte

**Aspekte:**
- Unrealistische Trainingserfolge („In 4 Wochen zur Traumfigur")
- Extreme Diäten und Ernährungsformen („Clean Eating", „Kein Zucker")
- Nahrungsergänzungsmittel und „Wunderpillen"
- Körperliche „Makel" und deren angebliche „Lösungen"
- Filter, Beleuchtung, Posen als Manipulation

**Differenzierung Sek I/II:**
- Sek I: Beschreiben, Erkennen von Unrealistischem, persönliche Reflexion
- Sek II: Zusätzlich: Geschäftsmodelle analysieren, gesellschaftliche Auswirkungen, Bodyshaming, Essstörungen

---

### Phase 5: Transfer & Abschluss – „Wahr oder Fake?"-Quiz (20 Min.)
**Ziel:** Gelerntes anwenden, spielerischer Wissenstransfer, Peer-Education

| Element | Beschreibung | Sozialform | Material |
|---------|--------------|------------|----------|
| Produktion | Partnerteams erstellen je 3-5 Quiz-Fragen mit Auflösung und kurzer Begründung | Partnerarbeit | Arbeitsblatt 5 |
| Quiz-Runde | Klasse spielt „Wahr oder Fake?" mit den besten Fragen | Plenum | Digital/Karten |
| Take-away | „Mein Faktencheck-Versprechen" – persönliche Reflexion | Einzelarbeit | Notizzettel |

**Quiz-Regeln:**
- Jede Frage braucht eine klare Antwort (Wahr/Fake/Differenziert) + kurze Begründung
- Bonus: Angabe einer verlässlichen Quelle

**Quiz-Beispiele (vorgegeben):**
- „Ananas verbrennt Fett" → **FAKE** (keine wissenschaftliche Grundlage)
- „Ausreichend Wasser trinken ist wichtig für die Gesundheit" → **WAHR** (aber keine feste Literanzahl)
- „Muskeln wiegen mehr als Fett" → **WAHR** (bei gleichem Volumen)
- „Kohlenhydrate machen dick" → **FAKE** (so pauschal, Energiebilanz entscheidet)
- „Sport macht glücklich" → **WAHR** (Endorphine, aber nicht übertreiben)

**Differenzierung Sek I/II:**
- Sek I: Multiple-Choice-Quiz mit klaren Antworten, Fokus auf „Wahr oder Fake?"
- Sek II: Offene Fragen mit Begründung und Quellenangabe, Kategorie „Differenziert" möglich

---

## 10. Materialübersicht

| Nr. | Material | Inhalt | Differenzierung |
|-----|----------|--------|-----------------|
| Bildmaterial | Einstieg | Collage: 8 Gesundheitsbehauptungen aus Social Media | einheitlich |
| AB 1 | Verbreitungsmechanismen | Tabelle: „Warum verbreiten sich Mythen?" | Sek I: 4 Gründe / Sek II: 6 Gründe + psychologische Erklärung |
| AB 2 | Faktencheck-Methoden | QQQQ-Methode, Quellencheck-Checkliste, Liste verlässlicher Quellen | Sek I: vereinfacht / Sek II: erweitert |
| AB 3a-d | Praxis-Faktencheck | 4 Gesundheitsbehauptungen zum Überprüfen | einheitlich, Anleitung differenziert |
| AB 4 | Influencer-Analyse | 3 fiktive Profile (Fitness/Ernährung/Beauty) mit Körperbild-Fokus | Sek I: Beschreibung / Sek II: + Geschäftsmodell-Analyse |
| AB 5 | Quiz-Erstellung | Vorlage für „Wahr oder Fake?"-Fragen mit Begründungsfeld | Sek I: Multiple Choice / Sek II: offene Fragen + Quellen |
| Exkurs | Hintergrundwissen | Wissenschaftliche Standards, verlässliche Gesundheitsquellen | primär Sek II |

---

## 11. Exkurs-Seite (Vorschlag)

**Thema:** „Wissenschaft vs. Meinung – Wie erkenne ich verlässliche Gesundheitsinformationen?"

**Inhalt:**
- Grundlagen wissenschaftlicher Methodik (kurz, verständlich)
- Unterschied: Einzelmeinung, Fallbericht, Studie, Meta-Analyse
- Peer-Review-Prozess und seine Bedeutung
- Interessenkonflikte in der Gesundheitsforschung (Wer bezahlt die Studie?)
- Verlässliche Quellen für Gesundheitsinformationen (Liste für Deutschland)

**Verlässliche deutsche Quellen (Beispiele):**
- gesundheitsinformation.de (IQWiG)
- patienten-information.de
- RKI (Robert Koch-Institut)
- DGE (Deutsche Gesellschaft für Ernährung)
- medizin-transparent.at
- Correctiv (Faktencheck)
- mimikama.at

---

## 12. Methoden und Sozialformen

| Sozialform | Anteil | Begründung |
|------------|--------|------------|
| Plenum | 30% | Impulse, Sicherung, Quiz-Runde, Diskussion |
| Gruppenarbeit | 30% | Faktenchecks, Influencer-Analyse, kooperatives Lernen |
| Partnerarbeit | 30% | Quiz-Erstellung, niedrigschwelliger Austausch |
| Einzelarbeit | 10% | Persönliche Reflexion |

**Methodische Prinzipien:**
- Arbeit mit fiktiven, aber realitätsnahen Social-Media-Beispielen
- Praktisches Anwenden von Faktencheck-Methoden (nicht nur Theorie)
- Keine Moralisierung von Körperbildern, sondern kritische Reflexion
- Digitale Tools optional, analog vollständig umsetzbar
- **Quiz als motivierende, spielerische Wissenssicherung** (Peer-Education)
- Körperbilder als roter Faden, nicht als isoliertes Thema

---

## 13. Qualitätskriterien (gemäß Leitlinien 2026)

| Kriterium | Umsetzung |
|-----------|-----------|
| **Binnendifferenzierung** | Durchgehend Sek I/Sek II-Varianten in allen Phasen und Materialien |
| **Klarer Fokus** | Eine Leitfrage: Fakten von Mythen unterscheiden – mit Körperbild-Bezug |
| **Andocken** | Baut auf LH6, LH19, LH8, LH7 auf – ohne inhaltliche Dopplung |
| **Gesundheitsbezug** | Körperliche & mentale Gesundheit zentral verankert |
| **Langlebigkeit** | Faktencheck-Methoden statt plattformspezifischer Details; Beispiele austauschbar |
| **Strukturelle Kontinuität** | Bewährter 5-Phasen-Aufbau (Einstieg → Transfer) |

---

## 14. Offene Fragen / Abstimmungsbedarf

1. **Arbeitstitel:** „Gesundheitsmythen im Netz" oder „Fakt oder Fake? Gesundheitsinformationen bewerten"?
2. **Bildmaterial:** Fiktive Mockups (rechtssicher) oder echte Screenshots (anonymisiert)?
3. **Video:** Gibt es vorhandenes Material zu Faktencheck-Methoden oder Neuproduktion nötig?
4. **Quiz-Tool:** Digital (z.B. Kahoot, Mentimeter, LearningApps) oder analog (Karten)?
5. **Exkurs-Thema:** Wissenschaftsmethodik oder lieber „Gefährliche Gesundheitstrends"?
6. **Influencer-Profile:** Vollständig fiktiv (empfohlen) – wie detailliert?
7. **Körperbilder-Integration:** Als roter Faden oder eigene Phase? (Konzept: beides)
8. **Aktualität:** Sollen aktuelle Mythen (z.B. zu KI-generierten Gesundheitstipps) einbezogen werden?

---

## 15. Nächste Schritte

1. [ ] Abstimmung Grobkonzept mit Redaktion
2. [ ] Recherche: Aktuelle Gesundheitsmythen in Social Media sammeln (TikTok, Instagram)
3. [ ] 4 Beispiel-Behauptungen für Faktenchecks auswählen und aufbereiten (mit Quellen)
4. [ ] 3 fiktive Influencer-Profile entwickeln (Fitness/Ernährung/Beauty) – mit Körperbild-Fokus
5. [ ] QQQQ-Methodenblatt und Checklisten erstellen (Sek I / Sek II)
6. [ ] Quiz-Vorlagen entwickeln (Sek I: Multiple Choice / Sek II: offen)
7. [ ] Exkurs-Seite ausarbeiten
8. [ ] Bildmaterial für Einstieg klären (Mockups erstellen)
9. [ ] Gegenlesen durch Fachlektorat
10. [ ] Layout-Erstellung nach Freigabe

---

## 16. Zusätzliche Ideen / Erweiterungsmöglichkeiten

### Optionale Vertiefung: KI und Gesundheitsmythen
- Neue Dimension: KI-generierte Gesundheitstipps (ChatGPT, Bild-KI für „Vorher-Nachher")
- Frage: Wie erkennt man KI-generierte Falschinformationen?
- Verbindung zu LH „Mensch vs. Maschine" (KI-Handreichung)

### Peer-Education-Erweiterung
- Quiz-Fragen der Klasse werden zu einem Klassenprojekt zusammengeführt
- Weitergabe an jüngere Klassen (Sek II erstellt Quiz für Sek I)
- Digitale Veröffentlichung auf Schulwebsite (optional)

### Elternbrief-Baustein
- Kurze Info für Eltern: „So sprechen Sie mit Ihrem Kind über Gesundheitsmythen"
- Verlässliche Quellen für Familien

---

*Stand: Januar 2026*  
*Erstellt im Rahmen von DURCHBLICKT! – Ein Programm der BARMER*
